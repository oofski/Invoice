...
VERIFY BEFORE RETURNING (iterate until this holds): the sum of every extracted line-item amount — counting discount/credit/refund lines as negative and including any freight/shipping line — PLUS the sales/use tax must equal the invoice total shown in the document. If they do not reconcile, keep iterating: re-scan every page for a line that was missed, truncated, split across a page break, or misread, and correct the amounts until they sum to the total. NEVER invent, pad, or adjust a line just to force a match — if after careful re-checking the document genuinely does not foot, extract exactly what is shown.`;
